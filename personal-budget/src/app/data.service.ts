...
getData(username): Observable<any> {
  ...
  const headers = {'content-type': 'application/json','Authorization' : `Bearer ${token}`};
    this.DataObservable = this.http.get('http://104.131.187.125:3000/budget',{ headers: headers,params:{userid : username }}).pipe(shareReplay());
...
addBudgetdata(data:BudgetSchema){
  ...
  console.log(body)
  return this.http.post('http://104.131.187.125:3000/budget',body,{'headers':headers});
}
...
private readonly NAMES = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'];
  ...
  userSignUp(data:UserSchema){
    ...
    const body=JSON.stringify(data);
    return this.http.post('http://104.131.187.125:3000/users',body,{'headers':headers});
  }
  ...
  userLogin(data:UserSchema){
    ...
    const body=JSON.stringify(data);
    console.log(body)
    return this.http.post('http://104.131.187.125:3000/auth',body,{'headers':headers}).subscribe((res:any)=>{
      console.log(res);       
